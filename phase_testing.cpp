...
int main() {
    ...
    vector<double> timing = generateTiming(SAMPLE_RATE, LENGTH);

    
    for (double phi = -PI; phi <= PI; phi += PHI_STEP) {
        
        vector<SignalComponent> sineWaveComponents = {{SIG_AMPL, SIG_FREQ, phi}};
        vector<double> sineWave = generateSignal(timing, sineWaveComponents);
